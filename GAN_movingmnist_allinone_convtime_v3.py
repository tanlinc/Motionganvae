...
# --- parameter ------------------------------------------------------------------------------
EXPERIMENT = "enc_convtime_3_v3"  # path to results folder to save/restore
...
def PostEncoder(input_time): # input:  [BATCH_SIZE, 4, 4, 4*DIM, TIMESTEPS]
    ...
    input_time_rs = tf.reshape(input_time, [wdn,wdx,wdy,wdt*wdc])
    w_np = np.zeros([1,1,wdc,TIMESTEPS,wdc])  # filter size = TIMESTEPS
    for ic in range (wdc):
        w_np[...,ic,:,ic] = np.ones(TIMESTEPS)/TIMESTEPS
    w_np = np.reshape(w_np,[1,1,TIMESTEPS*wdc,wdc])
    w = tf.constant(w_np, dtype=tf.float32)
    out = tf.nn.conv2d(input_time_rs,w,[1,1,1,1],"SAME", name='Enc.timeconv')  
