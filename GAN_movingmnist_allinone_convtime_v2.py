...
# --- parameter ------------------------------------------------------------------------------
EXPERIMENT = "enc_convtime_3_v2"  # path to results folder to save/restore
...
def PostEncoder(input_time): # input:  [BATCH_SIZE, 4, 4, 4*DIM, TIMESTEPS]
    ...
    input_time_t = tf.transpose(input_time, [0,1,2,4,3]) # NHWTC
    input_time_rs = tf.reshape(input_time_t, [wdn,wdx*wdy,wdt,wdc])
    w_np = np.zeros([1,TIMESTEPS,wdc,1])  # filter size = TIMESTEPS
    w_np[0,:,:,0] = np.ones([TIMESTEPS, wdc])/TIMESTEPS  # average over time
    w = tf.constant(w_np, dtype=tf.float32)
    out = tf.nn.depthwise_conv2d(input_time_rs,w,[1,1,1,1],"VALID", name='Enc.timeconv')  
...
if(MODE == 'enc'):
    ...
    code_time_3 = PreEncoder(time_data[:,0,:])
    code_time_4 = PreEncoder(time_data[:,1,:])
    code_time_5 = PreEncoder(time_data[:,2,:])
    code_time = tf.stack([code_time_3, code_time_4, code_time_5])  # (5, 50, 256, 4, 4) # code_time_1, code_time_2
...
if(MODE == 'enc'):
    ...
    fixed_code_time_3 = PreEncoder(fixed_time_data[:,0,:])
    fixed_code_time_4 = PreEncoder(fixed_time_data[:,1,:])
    fixed_code_time_5 = PreEncoder(fixed_time_data[:,2,:])
    fixed_code_time = tf.stack([fixed_code_time_3, fixed_code_time_4, fixed_code_time_5])  # (5, 50, 256, 4, 4) # fixed_code_time_1, fixed_code_time_2
    fixed_code_time_t = tf.transpose(fixed_code_time, [1,3,4,2,0], name='transpose_before_convtime')    # to: [BATCH_SIZE, 4, 4, 4*DIM, time_steps]
